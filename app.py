...
st.set_page_config(
    page_title="🅿️ Parking Tariff Analysis Dashboard",
    layout="wide", 
    initial_sidebar_state="expanded",
    menu_items={'About': "Progressive Parking Tariff Analysis Dashboard"}
)
...
st.markdown("""
    ...
    * {
    ...
    h1 {
        ...
        text-shadow: 0 4px 12px rgba(255, 140, 0, 0.2);
    }
...
def train_models(df, jam_cols):
    ...
    def build_model(X, y):
        ...
        # Using all data as reference (X_all)
...
def predict_single_input(jenis, hari, jam_input, jumlah_input, model, le, location_base_data): 
    ...
    try:
        ...
        # Build feature vector using location as base
        ...
        for feat in feature_names:
            if feat in location_dict:
                ...
                matching = [k for k in location_dict.keys() if feat.lower() in k.lower() or k.lower() in feat.lower()]
                val = location_dict[matching[0]] if matching else 0.0
...
def display_data_table(df_raw, df_processed):
    st.markdown("""
    <div style='padding: 30px; background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); border-radius: 16px; margin-bottom: 40px; box-shadow: 0 10px 35px rgba(79, 172, 254, 0.2); border: 1px solid rgba(255, 255, 255, 0.2); backdrop-filter: blur(10px);'>
        <h2 style='color: white; margin: 0; font-size: 2.2rem; font-weight: 800; letter-spacing: -0.5px;'>1️⃣ Data Management & Overview</h2>
        <p style='color: rgba(255,255,255,0.95); margin: 12px 0 0 0; font-size: 1.05rem;'>Explore raw and preprocessed parking data with statistics and summaries</p>
    </div>
    """, unsafe_allow_html=True)
...
def display_visualization(df, quantile_bounds, jam_cols, number_cols):
    ...
    <div style='padding: 30px; background: linear-gradient(135deg, #ff8c00 0%, #ff6b35 100%); border-radius: 16px; margin-bottom: 40px; box-shadow: 0 10px 35px rgba(255, 140, 0, 0.2); border: 1px solid rgba(255, 255, 255, 0.2); backdrop-filter: blur(10px);'>
        ...
        <p style='color: rgba(255,255,255,0.95); margin: 12px 0 0 0; font-size: 1.05rem;'>Comprehensive dashboard displaying parking density, revenue patterns, and quantile analysis</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with tab3:
        ...
        rata_jam = {}
        for kolom in jam_cols:
            rata_jam[kolom] = df[kolom].mean()

        data_visual = []

        for kolom in jam_cols:
            # Extract type, category, and day from column name
            # Expected format: "Peak Hours for Cars (Weekday)"
            # or "Peak Hours for Motorcycles (Weekday)"
            parts = kolom.split(' for ')
            
            if len(parts) != 2:
                continue
                
            category_part = parts[0]  # e.g., "Peak Hours"
            rest = parts[1]  # e.g., "Cars (Weekday)" or "Motorcycles (Weekday)"
            
            category = category_part.replace(' Hours', '')  # e.g., "Peak"
            
            # Extract vehicle type and day
            jenis = None
            hari = None
            
            if '(Weekday)' in rest:
                jenis = rest.replace(' (Weekday)', '')
                hari = 'Weekday'
            elif '(Weekend)' in rest:
                jenis = rest.replace(' (Weekend)', '')
                hari = 'Weekend'
            
            if jenis is None or hari is None:
                continue
            
            # Get average time for this category
            try:
                avg_time = pd.to_numeric(rata_jam[kolom], errors='coerce')
                if pd.isna(avg_time):
                    continue
            except:
                continue
            
            # Get load based on vehicle type and day
            kolom_jumlah = f'Number of {jenis} ({hari})'
            
            if kolom_jumlah not in df.columns:
                continue
            
            try:
                load_rata_kendaraan = pd.to_numeric(df[kolom_jumlah], errors='coerce').mean()
                if pd.isna(load_rata_kendaraan) or load_rata_kendaraan == 0:
                    load_rata_kendaraan = 1  # Use 1 as minimum to show data
            except:
                load_rata_kendaraan = 1
            
            # Apply load multiplier based on category
            if category == 'Peak':
                avg_load = load_rata_kendaraan * 1.0
            elif category == 'Moderate':
                avg_load = load_rata_kendaraan * 0.5
            elif category == 'Off-Peak':
                avg_load = load_rata_kendaraan * 0.2
            else:
                avg_load = load_rata_kendaraan
        ...
        if len(data_visual) == 0:
            ...
            # Plot Motorcycles (handle both singular and plural)
            moto_data = df_plot[df_plot['Type'].isin(['Motorcycle', 'Motorcycles'])]
            if len(moto_data) > 0:
                sns.barplot(
                    data=moto_data,
                    x='Combined',
                    y='Avg_Load',
                    order=[c for c in order_bar if c in moto_data['Combined'].values],
                    palette='viridis',
                    ax=axes[0]
                )
            ...
            # Plot Cars (handle both singular and plural)
            car_data = df_plot[df_plot['Type'].isin(['Car', 'Cars'])]
            if len(car_data) > 0:
                sns.barplot(
                    data=car_data,
                    x='Combined',
                    y='Avg_Load',
                    order=[c for c in order_bar if c in car_data['Combined'].values],
                    palette='magma',
                    ax=axes[1]
                )
...
def display_modeling(models_data):
    st.markdown("""
    <div style='padding: 30px; background: linear-gradient(135deg, #ff8c00 0%, #ff6b35 100%); border-radius: 16px; margin-bottom: 40px; box-shadow: 0 10px 35px rgba(255, 140, 0, 0.2); border: 1px solid rgba(255, 255, 255, 0.2); backdrop-filter: blur(10px);'>
        <h2 style='color: white; margin: 0; font-size: 2.2rem; font-weight: 800; letter-spacing: -0.5px;'>3️⃣ Tariff Potential Classification Modeling</h2>
        <p style='color: rgba(255,255,255,0.95); margin: 12px 0 0 0; font-size: 1.05rem;'>Random Forest Classification - Predicting parking tariff potential for motorcycles and cars</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    def display_model_results(jenis, data):
        ...
        with col2:
            ...
            for i, v in enumerate(importance.values):
                ax_imp.text(v + 0.002, i, f'{v:.4f}', va='center', fontsize=11, fontweight='bold', color='black')
            
            ax_imp.set_title(f'Top 5 Feature Importance - {jenis}', fontsize=14, fontweight='bold')
            ax_imp.set_xlabel('Importance Score', fontsize=12)
            ax_imp.set_xlim(0, max(importance.values) * 1.15)  # Add space for labels
            st.pyplot(fig_imp)
...
def display_map_and_simulation(df_long, map_center, models_data, df_spatial):
    ...
    <div style='padding: 20px; background: linear-gradient(135deg, #fa709a 0%, #fee140 100%); border-radius: 10px; margin-bottom: 30px;'>
        ...
        <p style='color: rgba(255,255,255,0.95); margin: 10px 0 0 0;'>📍 Interactive map visualization and tariff simulation for different parking locations</p>
    </div>
    """, unsafe_allow_html=True)
    
    st.subheader("🗺️ Static Tariff Potential Prediction Map")
    st.info("📌 Location markers show parking potential classification. Select a location below to simulate progressive tariff strategies.")
...
st.markdown("""
<div style='padding: 25px; background: linear-gradient(135deg, #ff8c00 0%, #ff6b35 100%); border-radius: 14px; margin-bottom: 30px; box-shadow: 0 10px 30px rgba(255, 140, 0, 0.2); border: 1px solid rgba(255, 255, 255, 0.2); backdrop-filter: blur(10px);'>
    <p style='color: white; font-size: 1.05rem; margin: 0; font-weight: 500;'>
        📊 <b>Advanced Dashboard for parking tariff potential classification using Machine Learning</b><br>
        🎯 Analyze parking density, revenue patterns, and implement data-driven tariff optimization strategies
    </p>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    st.markdown("""
    <div style='padding: 20px; background: linear-gradient(135deg, #ff8c00 0%, #ff6b35 100%); border-radius: 12px; margin-bottom: 25px; box-shadow: 0 8px 25px rgba(255, 140, 0, 0.15); border: 1px solid rgba(255, 255, 255, 0.2);'>
        <h3 style='color: white; margin: 0; font-size: 1.4rem; font-weight: 800;'>📊 Navigation Menu</h3>
        <p style='color: rgba(255, 255, 255, 0.85); margin: 8px 0 0 0; font-size: 0.9rem;'>Select a section to explore</p>
    </div>
    """, unsafe_allow_html=True)
